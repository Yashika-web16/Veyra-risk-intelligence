  ...
  async function runRiskSimulation() {
    try {
      ...
      if (!response.ok || !data.success) {
        throw new Error(data.error || "Risk simulation failed.");
      }
      ...
      setSimulationError(
        error.message || "Unable to generate projection."
      );
    } finally {
  ...
  return (
    <main className="min-h-screen bg-[#05070a] text-white">
      <div className="mx-auto max-w-[1500px] px-6 py-8 lg:px-10">
        ...
        <section className="mt-8 overflow-hidden rounded-2xl border border-white/[0.07] bg-[#090c11]">
            ...
            <div className="flex flex-col justify-between gap-4 sm:flex-row sm:items-center">
              <div>
                <div className="flex items-center gap-2">
                  ...
                  <span className="rounded-full border border-emerald-400/10 bg-emerald-400/5 px-2 py-1 text-[9px] uppercase tracking-widest text-emerald-300">
                    Scenario Engine
                  </span>
                </div>

                <p className="mt-2 max-w-2xl text-xs leading-5 text-slate-600">
                  Simulate how this entity&apos;s risk could evolve if current
                  behaviour continues.
                </p>
              </div>
              ...
              <button
                ...
                className="rounded-xl bg-emerald-400 px-4 py-2.5 text-xs font-semibold text-black transition hover:bg-emerald-300 disabled:cursor-not-allowed disabled:opacity-50"
              >
                {loadingSimulation
                  ? "Simulating..."
                  : simulation
          ...
          {!simulation && !loadingSimulation && !simulationError && (
            <div className="px-6 py-10 text-center">
              <div className="mx-auto flex h-12 w-12 items-center justify-center rounded-full border border-white/[0.07] bg-white/[0.02] text-xl text-slate-600">
                ◌
              </div>

              <p className="mt-4 text-sm text-slate-500">
                See how this risk profile could evolve over the next 72 hours.
              </p>

              <p className="mt-2 text-[10px] text-slate-700">
                Scenario analysis only — not a guaranteed forecast.
              </p>
            </div>
          )}
          ...
          {loadingSimulation && (
            <div className="px-6 py-12 text-center">
              ...
              <p className="mt-4 text-xs text-slate-500">
                Analysing current risk signals...
              </p>
            </div>
          )}
          ...
          {simulation && (
            <div className="p-5 sm:p-6">
              <div className="grid gap-3 md:grid-cols-3">
                ...
                <div className="rounded-xl border border-white/[0.06] bg-[#06080c] p-4">
                  <div className="text-[10px] uppercase tracking-widest text-slate-600">
                    Current
                  </div>

                  <div className="mt-4 text-3xl font-semibold text-white">
                    {simulation.baseline.risk}
                  </div>

                  <div className="mt-1 text-[10px] uppercase tracking-wider text-red-300">
                    {getRiskLabel(simulation.baseline.risk)}
                  </div>
                  ...
                  <div className="mt-5 space-y-2 text-xs">
                    <div className="flex justify-between">
                      ...
                      <span className="text-slate-300">
                        {simulation.baseline.connections}
                      </span>
                    </div>
                ...
                <div className="rounded-xl border border-white/[0.06] bg-[#06080c] p-4">
                  ...
                  <div className="mt-4 text-3xl font-semibold text-white">
                    {simulation.projections[0]?.risk ?? "--"}
                  </div>

                  <div className="mt-1 text-[10px] uppercase tracking-wider text-red-300">
                    {simulation.projections[0]
                      ? getRiskLabel(simulation.projections[0].risk)
                      : "—"}
                  </div>
                  ...
                  <div className="mt-5 space-y-2 text-xs">
                    <div className="flex justify-between">
                      <span className="text-slate-600">Connections</span>
                      <span className="text-slate-300">
                        {simulation.projections[0]?.connections ?? "--"}
                      </span>
                    </div>
                ...
                <div className="rounded-xl border border-red-500/10 bg-red-500/[0.025] p-4">
                  ...
                  <div className="mt-4 text-3xl font-semibold text-white">
                    {simulation.projections[1]?.risk ?? "--"}
                  </div>

                  <div className="mt-1 text-[10px] uppercase tracking-wider text-red-300">
                    {simulation.projections[1]
                      ? getRiskLabel(simulation.projections[1].risk)
                      : "—"}
                  </div>
                  ...
                  <div className="mt-5 space-y-2 text-xs">
                    <div className="flex justify-between">
                      <span className="text-slate-600">Connections</span>
                      <span className="text-slate-300">
                        {simulation.projections[1]?.connections ?? "--"}
                      </span>
                    </div>
              ...
              <div className="mt-4 grid gap-3 sm:grid-cols-3">
                <div className="rounded-xl border border-white/[0.05] bg-[#06080c] p-4">
                  ...
                  <div className="mt-2 text-lg font-semibold text-red-300">
                    +{simulation.summary.riskIncrease}
                  </div>
                ...
                <div className="rounded-xl border border-white/[0.05] bg-[#06080c] p-4">
                  ...
                  <div className="mt-2 text-lg font-semibold text-orange-300">
                    +{simulation.summary.connectionIncrease}
                  </div>
              ...
              <div className="mt-4 rounded-xl border border-amber-500/10 bg-amber-500/[0.025] p-4">
                  ...
                  <div>
                    ...
                    <p className="mt-2 text-xs leading-6 text-slate-500">
                      ...
                      <span className="text-amber-300">
                        {simulation.summary.severity.toLowerCase()}
                      </span>{" "}
                      risk trajectory if current behaviour continues.
                      Network growth and exposure may become increasingly
